...
2026年9月26日に提出された記録を確認しました。環境名・OSの版は利用者申告です。
Linuxのcommit.txtはmainへのマージコミット `ecf4bd2105c2986a8aea38ebbf6b14c8ae2302ad` と一致し、起動ログもv1.10.1です。
Synologyは起動ログでv1.10.1を確認しましたが、コミット番号の記録は含まれていません。

| 環境 | 記録で確認できた結果 |
| --- | --- |
| DS1522+ Container Manager、Docker 24.0.2、linux/amd64 | 2アカウントへの期限到達時の通知成功、定期IP取得、処理停滞の検知、自動復帰、最終healthy・連続失敗0 |
| DS1522+ VM、Ubuntu 24、Linux直接実行 | 2アカウントの通知成功、復帰後の5分周期のIP取得、一時停止からの自動復帰、後の起動ログと復帰タイマーの実行記録・次回予定 |

Synologyのbefore.txtとafter.txtは同じコンテナIDで、再起動回数が0から1へ増えています。
日本時間00:45:01にRESTART_ATTEMPT、00:45:05にv1.10.1起動、00:46:01にRECOVEREDが記録されています。
00:25には両アカウントの通知成功があり、再起動後は期限前としてSKIPしています。

Linuxは日本時間07:00:18にSIGSTOP、07:06:04に再起動要求と新しい起動、07:06:35にRECOVEREDが記録されています。
その後もIP取得を続け、再起動後の記録として提出されたupdater.logには07:30の起動と両アカウントの通知成功があります。
timer.txtには実行済み時刻と次回予定があります。ブートIDやOS情報そのものは今回の記録に含まれず、OS再起動操作をログだけで独立に証明したものではありません。

**今回の実環境試験に含まれないもの：** 監視不能のPROBE_UNAVAILABLE・判定回復のPROBE_RECOVERED、復帰の回数上限・待機時間の境界、過大応答などの異常通信は自動テストで確認しています。今回の実機ログで再現したとは扱いません。
Ubuntu 26と通常のUbuntu Dockerのv1.10.1再試験、ARMの確認は未実施です。過去の版の結果は以下に分けて残します。

修正コードの[PR #17のCI](https://github.com/Karoeba/mydns-updater/actions/runs/36067117937)は、対象 `37eb0142ba742423442dcf72dcd0c7d2fba399c5` で全9ジョブ成功です。上の実環境試験とは別の証拠です。
