...
この資料はv1.9.0の実装前に行った方式検証の記録です。
導入と今回追加した機能の確認は、[Docker・Synologyの自動復帰](../docker-recovery.md)を参照してください。

利用者のDS1522+（Docker 24.0.2）でも、方式確認の7項目すべてが成功しました。受領したrecovery-probe.logで確認しています。
定期実行・回数制限を組み込んだv1.9.0全体のNAS実機確認とは別の結果です。
