...
Dockerはupdate.shとlibを構築時に/appへコピーします。Linuxは同じコードを所定のフォルダー内へ配置します。
.dockerignoreは構築に必要なファイルだけを許可します。実設定・state・復帰履歴・ログ・テストはイメージに含めません。
Dockerでコードを変えたら必ず再構築・再作成します。旧方式の/appへのマウントは、イメージ内のコードを隠すため残しません。
...
tests/test-image-package.pyは格納されたコード、秘密情報を含めない構成、旧マウントからの移行・切り戻し、設定・stateの保持を確認します。ホスト側の復帰処理も、構築したイメージを使って確認します。
