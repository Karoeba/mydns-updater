...
ログはJST固定です。起動時にバージョンと実効設定値を表示します。起動後の設定変更で過去の起動ログは変わりません。IP不変・更新期限前は、通常ログが増えなくても正常です。
...
GitHub ActionsはPR作成・更新時とmainへのpush時に、30項目の模擬テストと3項目の設定再読み込みテストを実行します。Actionsの「Docker tests」から手動実行もできます。結果はPRのChecksとActionsログ、成果物 `test-reports`（14日間保存）で確認できます。コンテナ起動前の失敗ではレポートがない場合があります。
...
模擬テストは外部通信を無効にし、実アカウントを使いません。初回のイメージ取得には接続が必要です。結果は `tests/reports` に保存され、成功時は `ALL TESTS PASSED (30 checks)` と表示して終了します。途中の失敗ログは異常系テストに含まれるため、最後の結果を確認してください。
...
DS1522+で30項目の合格と、実アカウントの通知・アカウント別定期更新・設定再読み込み・デバッグ切替を確認済みです。GitHubのテストだけではNAS上の動作は保証されないため、導入先で起動・通信・状態保持を確認してください。テスト失敗時のマージ禁止には別途リポジトリ設定が必要です。
