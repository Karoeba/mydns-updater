...
| 区分 | 設定 | 内容 | 既定値・範囲 |
| --- | --- | --- | --- |
| 共通 | CHECK_INTERVAL | IPv4確認後の待機時間 | 300秒、60〜86400秒 |
| 共通 | FORCE_UPDATE_INTERVAL | 前回の通知成功から再通知するまでの時間 | 86400秒、3600〜604800秒 |
| 共通 | TZ | ログのタイムゾーン | Asia/Tokyo |
| 共通 | DEBUG | 詳細ログの表示 | 0（無効）、1で有効 |
| 共通 | IP_CHECK_URL1〜3 | IPv4取得先 | 下記の標準サービス |
| アカウント | ID | MyDNSのMasterID | アカウントごとに必須 |
| アカウント | PASSWORD | MasterIDに対応するパスワード | アカウントごとに必須 |
| アカウント | DOMAIN | ログ表示用のドメイン名 | アカウントごとに必須 |
